...
=== 🔴 命题思维（你必须先理解命题逻辑，再动笔） ===
在你动手写题之前，请先思考以下问题，并将答案融入你的命题：

1. 这条路线有什么"亮点"？
   - 有哪几步是高考常考的核心反应（硝化、还原、酯化、水解、酰化、缩合等）？
   - 有无特殊的人名反应或非常规转化，可作为第(5)题已知信息的来源？
   - 路线的关键中间体是什么？哪一步是最关键的转化？

2. 各小题如何与路线紧密结合？
   - 第(1)题：从路线中选一个具体中间体，问其官能团名称、反应类型或分子式
   - 第(2)题：选路线中某一步的转化，让学生写出反应方程式或推断产物结构简式
   - 第(3)题：选路线中需要特定试剂/条件的一步，问反应条件或试剂选择理由
   - 第(4)题：选路线中某关键中间体的同分异构体，条件要具体、有区分度
   - 第(5)题：设计与路线"相似但不同"的合成路线（不同起始原料/不同中间体/不同反应顺序）

3. 已知信息如何设计？
   - 优先使用路线中的人名反应或特殊转化作为已知信息
   - 若路线没有特殊反应，从高中课本未涉及但学生能理解的经典反应中选择
   - 已知信息必须包含具体反应方程式（→[条件]格式），不能泛泛而谈

4. 各小题之间要有逻辑递进：
   - 低分题考查识记（官能团、反应类型）→ 中分题考查理解（方程式、条件选择）→ 高分题考查应用（同分异构体推理、合成路线设计）
   - 后面的小题可以引用前面小题的结论（如第3题基于第2题的结构推断结果）

=== 🔴 命题原创性要求（避免模板化！） ===
每次出题都要有新鲜感，不能让题目看起来像套模板。以下是具体方法：

1. 第(1)题多样化：
   - 不要总是问"化合物X的官能团名称"
   - 可变式：命名某化合物、写出分子式、判断手性碳个数、命名某官能团、指出某反应的反应类型
   - 示例："化合物C的分子式为____"、"B→C的反应类型为____"、"化合物D中手性碳原子的个数为____"

2. 第(2)题多样化：
   - 不要总是问"写出B→C的化学方程式"
   - 可变式：根据条件推断产物结构简式、根据产物推断反应物、指出某步反应中试剂的作用
   - 示例："D的结构简式为____"、"由C转化为D的化学方程式为____"

3. 第(3)题多样化：
   - 不要总是问"X→Y所需的试剂和条件"
   - 可变式：选择正确的试剂组合、比较不同试剂的选择性、解释为什么选择某试剂
   - 示例："下列试剂中，可用于实现C→D转化的是____（填序号）"

4. 第(4)题同分异构体条件必须具体且多样化：
   - 条件①官能团特征：不要总是"遇FeCl₃显紫色"，可变式为"能发生银镜反应""能与NaHCO₃反应放出CO₂""能使溴水褪色""能发生水解反应"
   - 条件②谱学数据：必须有具体数值！如"峰面积比为1:2:2:3"或"有4组峰"或"红外光谱显示有羰基吸收峰"
   - 条件③结构限制：可变式为"苯环上一氯代物只有2种""含有2个手性碳""苯环上有3个取代基""不含甲基"

5. 第(5)题目标产物必须原创：
   - 目标产物是路线中某化合物的"类似物"（结构相近但不同），不能完全相同
   - 起始原料要多样化：苯、甲苯、苯酚、苯甲醛、苯甲酸、环己酮、丙酮等
   - 答案路线必须与题干路线不同，至少有3步不同
...
{{
  ...
  "stem": "题干（仅一句话，≤50字。只描述该化合物的实际应用意义。禁止包含路线步骤、→箭头、条件方括号、'合成路线如下'等）",
  "questions": [
...
第(1)题：官能团名称/反应类型/分子式（针对路线中具体化合物设问）
  - 🔴 必须指定路线中的具体化合物代号（如"化合物C中含有的官能团名称为____"），不能泛泛问"含有什么官能团"
  - 可问某步反应的反应类型（如"B→C的反应类型为____"）

第(2)题：结构简式推断或反应方程式书写（用{{{{结构式:SMILES}}}}标结构式）
  - 🔴 必须具体到路线中某一步的转化，让学生写出完整的反应方程式
  - 🔴 方程式格式必须与题干路线图一致：{{{{结构式:SMILES}}}}→[条件] {{{{结构式:SMILES}}}}，条件写在箭头上方方括号内
  - 🔴 不得使用传统 A＋B→C 格式，不得使用文字描述反应
  - 🔴 如果条件有多步，用(1)(2)编号，有(2)必须有(1)
  - 好的设问示例："写出B→C反应的化学方程式"、"写出由C制备D反应的化学方程式"

第(3)题：反应所需试剂或条件选择（与路线实际反应对应）
  - 🔴 必须具体到路线中某一步（如"B→C所需的试剂和条件为____"），不能泛泛而问
  - 若答案含方程式，也必须用路线图格式：A→[条件]B
  - 好的设问示例："C→D所需的试剂和条件为____"、"由E制备F可选用的试剂是____"

第(4)题：同分异构体，3个递进条件：①官能团特征反应 ②核磁共振氢谱 ③苯环取代位置/手性碳
  - 🔴 必须指定是路线中哪个化合物的同分异构体（如"化合物D的同分异构体中"）
  - 🔴 条件①必须是具体的官能团特征反应（如"遇FeCl₃溶液显紫色""能发生银镜反应""能与NaHCO₃反应放出CO₂"），不能泛泛说"含有某种官能团"
  - 🔴 条件②必须包含具体的谱学数据（如"核磁共振氢谱显示有4组峰，峰面积比为1:2:2:3"），不能泛泛说"氢谱有几组峰"
  - 🔴 条件③必须包含具体的结构限制（如"苯环上的一氯代物只有2种""含有1个手性碳原子""苯环上有两个取代基且处于对位"）
  - 三个条件必须能唯一确定目标结构，不能存在歧义

第(5)题：合成路线设计，必须含"已知"信息（具体反应方程式），明确写"制备化合物X"，答案5-7步
  - 🔴 已知信息必须用路线图格式：{{{{结构式:SMILES}}}}→[条件] {{{{结构式:SMILES}}}}
  - 🔴 答案每步必须用 →[条件] 格式，条件在箭头上方
  - 🔴 目标产物必须是路线中某化合物的"类似物"（结构相近但不同），不能和路线最终产物完全相同
  - 好的设问示例："已知：{{结构式:...}}→[...] {{结构式:...}}。请以甲苯为原料，设计制备化合物G的合成路线。"
...
第(1)题 — 基础识记（2分，easy）：
  ...
  - ❌ 差的设问：
    "该路线中涉及哪些官能团"（太泛）
    "化合物A的名称是什么"（A是起始原料，无意义）
    "写出反应类型"（未指定哪步反应）

第(2)题 — 结构推断/方程式（2分，easy）：
  - 🔴 必须具体到路线中某一步的转化
...
第(3)题 — 反应条件/试剂选择（2或3分，medium）：
  - 🔴 必须具体到路线中某一步
  - ✅ 好的设问：
    "C→D所需的试剂和条件为____"
    "由E制备F可选用的试剂是____（填序号）"
    "实现B→C转化，下列试剂中最合适的是____"
  - ❌ 差的设问：
    "该路线需要什么试剂"（太泛）
    "反应条件是什么"（未指定哪步反应）
...
第(4)题 — 同分异构体（3分，hard）：
  ...
  - 🔴 3个条件必须递进，且能唯一确定结构
  - ✅ 条件①示例（官能团特征）：
    "遇FeCl₃溶液显紫色" / "能发生银镜反应" / "能与NaHCO₃反应放出CO₂" / "能使溴的四氯化碳溶液褪色" / "能发生水解反应且水解产物之一能与FeCl₃显色"
  - ✅ 条件②示例（谱学数据）：
    "核磁共振氢谱显示有5组峰，峰面积比为1:2:2:2:1" / "核磁共振氢谱显示有3组峰" / "红外光谱显示有酯羰基吸收峰"
  - ✅ 条件③示例（结构限制）：
    "苯环上的一氯代物只有2种" / "含有1个手性碳原子" / "苯环上有3个取代基且各不相同" / "分子中不含甲基"
  - ❌ 差的条件：
    "含有苯环"（太泛，几乎所有同分异构体都含苯环）
    "能发生取代反应"（几乎所有有机物都能发生）
    "核磁共振氢谱有几组峰"（没有具体数据）
    "分子式为C₈H₈O₂"（分子式题干已经给出）

第(5)题 — 合成路线设计（5分，hard）：
  - 🔴 已知信息必须包含→[条件]格式的具体反应方程式
  - 🔴 已知信息中的反应必须是题干路线中未出现的新反应
  - 🔴 必须明确写出"制备化合物X"（X=目标产物代号）
  - ✅ 好的设问：
    "已知：{{结构式:...}}→[...] {{结构式:...}}。请以甲苯为原料，设计制备化合物G的合成路线（用流程图表示，其他无机试剂任选）。"
  - ❌ 差的设问：
    "已知格氏试剂能与醛酮反应。请设计合成路线"（已知信息太泛，没有具体反应式）
    "请合成化合物G"（缺少"制备/合成"关键词）
