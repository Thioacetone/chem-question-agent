...
=== 🔴 方程式和已知信息格式统一（最高警告） ===
整个题目中所有化学方程式（第2题答案、第5题答案、已知信息）必须使用与路线图完全一致的格式：
✅ 正确格式：{{{{结构式:SMILES}}}}→[条件] {{{{结构式:SMILES}}}}
✅ 多步格式：{{{{结构式:SMILES}}}}→[条件1] {{{{结构式:SMILES}}}}→[条件2] {{{{结构式:SMILES}}}}
❌ 禁止格式：A＋B→C 或 A＋B→[条件]C 或 A→B（无条件）
❌ 禁止格式：文字描述反应 如"苯甲醛与乙酸酐在碱催化下反应生成肉桂酸"
...
第(2)题：结构简式推断或反应方程式书写（用{{{{结构式:SMILES}}}}标结构式）
  - 方程式格式必须与题干路线图一致：A→[条件]B，条件写在箭头上方方括号内
  - 不得使用传统 A＋B→C 格式，统一用箭头→[条件]格式
...
- 所有化学方程式统一使用路线图格式：A→[条件]B（条件在箭头上方），禁止使用A＋B→C格式
- 第(5)题答案路线必须与题干路线不同
- 第(5)题答案条件必须极简！只写试剂名+条件符号（如△），逗号分隔，严禁"在...条件下""加热回流""催化""反应"等冗余词
- 第(5)题答案多步条件用(1)(2)编号，有(2)必须有(1)
