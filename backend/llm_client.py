...
{{
  ...
  "answers": [
    ...
    {{"number": 2, "content": "答案", "scoring_points": ["踩分点"]}},
...
第(5)题：合成路线设计，必须含"已知"信息（具体反应方程式），明确写"制备化合物X"，答案4-6步

=== 第(5)题答案格式（高考标准） ===
答案每步一行，用箭头表示反应，条件写在箭头上方方括号内：
...
- NaBH₄                              （还原）
- SOCl₂                               （酰氯化）
- (1) O₃ (2) Zn, H₂O               （烯烃臭氧化-还原）
- (1) NaOH, H₂O, △ (2) H⁺         （先碱水解后酸化）

记住：高考真题的条件就长这样！没有"加热回流""催化""反应""溶液中""作用下"等词语。
△就是加热符号，不要再写"加热""△加热"等。
=== 多步条件标号规则（重要！） ===多步反应使用(1)(2)(3)编号，渲染时：- (1) 显示在箭头上方- (2) 显示在箭头下方- (3) 也显示在箭头下方（与(2)同侧）- 有(2)必须有(1)，不能只有(2)没有(1)示例：- (1) O₃ (2) Zn, H₂O    →  (1)在上方，(2)在下方- (1) NaNO₂, HCl, 0-5℃ (2) H₂O, △  →  (1)在上方，(2)在下方- (1) LiAlH₄, 无水乙醚 (2) H₂O, H⁺ (3) NaOH  →  (1)在上方，(2)(3)在下方
...
示例：
"已知：{{结构式:O=Cc1ccccc1}}与{{结构式:CC(=O)OC(C)=O}}在碱催化下发生Perkin反应：{{结构式:O=Cc1ccccc1}}＋{{结构式:CC(=O)OC(C)=O}}→[碱] {{结构式:O=C(O)C=Cc1ccccc1}}"

=== 硬性约束 ===
- 化合物用字母代号（A、B、C...），禁止长系统名（仅"苯""甲苯""苯酚""乙酸"等极简名可用）
- 题干中不重复描述合成路线，路线图单独展示
...
- SMILES规范：苯环c1ccccc1，羧基C(=O)O，硝基N(=O)=O，氨基N，羟基O，醛基C=O
