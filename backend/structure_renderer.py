...
只输出SMILES字符串："""
    ...
    def render_route_diagram_svg(
        ...
        # === 判断是否换行及分行规则 ===
        # n = 化合物数量 = 步数 + 1
        # 5个化合物以内单行；6-10个化合物双行U字形：
        #   5步(6化合物): 3+3    6步(7): 4+3    7步(8): 4+4
        #   8步(9): 5+4          9步(10): 6+4
        if n <= 5:
            rows = [(0, n)]
        elif n == 6:
        ...
        def _split_reagent(text):
            ...
            for p in raw_parts:
                ...
                if m:
                    ...
                    numbered_parts.append((num, content))
                elif numbered_parts:
                    numbered_parts.append((numbered_parts[-1][0] + 1, p.strip().rstrip(';,， ').strip()))
                else:
                    numbered_parts.append((1, p.strip().rstrip(';,， ').strip()))

            if len(numbered_parts) >= 2:
                # 重新编号确保从(1)开始连续
                renumbered = []
                for idx, (_, content) in enumerate(numbered_parts):
                    renumbered.append(f"({idx+1}) {content}")
                above = [renumbered[0]]
                below = renumbered[1:]
                return {"above": above, "below": below}

            # 空格分隔的(1)xxx (2)yyy格式（无分号）
            space_numbered = re.findall(r'\((\d+)\)\s*([^()]+?)(?=\(\d+\)|$)', text)
            if len(space_numbered) >= 2:
                above = [f"({space_numbered[0][0]}) {space_numbered[0][1].strip()}"]
                below = [f"({n}) {c.strip()}" for n, c in space_numbered[1:]]
                return {"above": above, "below": below}
